...
def data_description():
    ...
    st.write("- **Những phương tiện có trong tập dữ liệu:** Xe hơi Ấn Độ, Xe tải Ấn Độ, Xe buýt, Xe khách Tempo, Máy kéo, Xe hơi, Xe hai bánh")
    ...
    class_names = list(dataset_config['names'].values())  # ['auto', 'bicycle', ...]
    ...
    ax.bar(['Train', 'Val', 'Test'], [train_count, val_count, test_count], color=['blue', 'orange', 'green'])
...
def analyze_dataset(train_dir, class_names):
    ...
    display_sample_images_with_boxes(train_dir, train_label_dir, 'Train', class_names)
...
def display_sample_images_with_boxes(image_dir, label_dir, title, class_names, num_samples=4):
    ...
    for i, img_file in enumerate(sample_files):
        ...
        for box, label in zip(boxes, labels):
            ...
            axes[i].text(box[0], box[1], label, color='red', fontsize=10, bbox=dict(facecolor='white', alpha=0.5))
...
def plot_pixel_distribution(image_dir):
    ...
    for i, color in enumerate(['red', 'green', 'blue']):
        ax.hist(img_array[:, :, i].ravel(), bins=256, color=color, alpha=0.5, label=color.capitalize())
