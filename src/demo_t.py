...
def demo_detection():
    ...
    try:
        ...
        if os.path.exists(yaml_path):
            ...
            class_names = ['auto', 'bicycle', 'bus', 'car', 'tempo', 'tractor', 'two_wheelers', 'vehicle_truck']
    except Exception as e:
        st.error(f"Error loading class names: {str(e)}")
        class_names = ['auto', 'bicycle', 'bus', 'car', 'tempo', 'tractor', 'two_wheelers', 'vehicle_truck']
    
    # Confidence threshold
    conf_threshold = st.slider("Ngưỡng Confidence", min_value=0.1, max_value=1.0, value=0.25, step=0.05)
    
    # Input options
    st.write("### Chọn nguồn đầu vào")
    input_option = st.radio("Chọn nguồn", ["Upload ảnh", "Upload video", "Camera"])
    ...
    if input_option == "Upload ảnh":
        ...
        if uploaded_file is not None:
            ...
            if results is not None:
                ...
                st.image(output_image, caption="Kết quả", use_column_width=True)
        ...
        if uploaded_file is not None:
            ...
            st.write(f"Video FPS: {fps}, Resolution: {frame_width}x{frame_height}")
            ...
            if ret:
                ...
                # Display frame
                video_placeholder.image(output_frame, caption="Frame preview", use_column_width=True)
